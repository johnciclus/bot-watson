import {app, express, appEnv} from './server'
...
function receivedMessage(event) {
    ...
    if (isEcho) {
        ...
        if(messageAttachments[0].type == 'location'){
            ...
            let image_url = messageAttachments[0].payload.url;
            let imageProcessing = payloadRules.get('Image');

            if(imageProcessing){
                imageProcessing(senderID, image_url);
            }
        }
...
module.exports = {app, express, rules, payloadRules, buffer, listener, limit, defaultSearch, callSendAPI, sendTypingOn, sendTypingOff, setListener, getListener, deleteListener, setDataBuffer, testAPI};
